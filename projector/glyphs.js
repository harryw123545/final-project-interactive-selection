 ...
 class Glyphs {
     ...
     display() {
         ...
         for (let k = 0; k < a; k++) {
             ...
             push(); // Push and pop to avoid applying the style to the rest of the shape
             strokeWeight(5);
         ...
         for (let d = 0; d < genes.length; d++) {
             let size = 3;
             let c = floor(map(genes[d], 0, 1, 97, 122)); // Map every element of the array to letters in the ASCII table
             let tw = width * 0.08 / size;
             let th = height * 0.08 / size;
             let y1 = (d % size) * th; // Use modulo to divide every letter evenly in a grid
             let x1 = parseInt(d / size) * tw;

             push();
             translate(width / 2 + width / 5.5, height / 1.5); // Translate to the right of the creature
             textSize(15);
             text(String.fromCharCode(c), x1, y1); // Convert numbers into a series of letters
             pop();
         }
     }
 }